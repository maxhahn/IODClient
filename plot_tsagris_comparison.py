...
_df = _df.rename(
    {"federated_p_values": "Federated", "fisher_p_values": "Meta-Analysis"}
)
for i in [1, 3, 5, 7]:
    __df = _df.filter(pl.col("num_clients") == i)
    # print(i, len(_df), len(__df))
    __df = __df.sample(min(subsample_size, len(__df)))
    ...
    _render.savefig(
        f"images/pval_comp/scatter-c{i}-samples{num_samples}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )

    __df = _df.filter(pl.col("num_clients") == i).filter(
        pl.col("baseline_p_values") < 0.1
    )
    # print(i, len(_df), len(__df))
    __df = __df.sample(min(subsample_size, len(__df)))
    ...
    _render = hv.render(_plot, backend="matplotlib")
    _render.savefig(
        f"images/pval_comp/scatter-c{i}-samples{num_samples}-small.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
...
def get_correlation(df, identifiers, colx, coly):
    ...
    df_correlation_fix = df.with_columns(
        correct=(pl.col(colx) - pl.col(coly)).round(8) == 0
    )
    ...
    df_correlation_fix2 = df_correlation_fix2.filter(
        (pl.col(f"{colx}_nunique") == 1) | (pl.col(f"{coly}_nunique") == 1)
    )
    ...
    df_correlation_fix2 = df_correlation_fix2.with_columns(
        correlation_fix=(
            (pl.col("mean_difference_p_value") < 1e-4)
            & (pl.col("mean_correctness") > 0.9)
        ).cast(pl.Float64)
    )
    df_correlation_fix2 = df_correlation_fix2.drop(
        "mean_difference_p_value", "mean_correctness"
    )
    ...
    _df = _df.with_columns(
        pl.coalesce(["p_value_correlation", "correlation_fix"])
    ).drop("correlation_fix")
...
df_fed = get_correlation(
    df, identifiers, "federated_p_values", "baseline_p_values"
).rename({"p_value_correlation": "Federated"})
df_fisher = get_correlation(
    df, identifiers, "fisher_p_values", "baseline_p_values"
).rename({"p_value_correlation": "Meta-Analysis"})
...
for i in [1, 3, 5, 7]:
    _plot = (
        df_unpivot.filter(pl.col("num_clients") == i)
        .sort("Method", "# Samples")
        .hvplot.line(
            x="# Samples",
            y="Correlation",
    ...
    _plot = _plot.opts(
        legend_opts={
            ...
            "bbox_to_anchor": (0.95, 0.001),  # fine-tune vertical placement
            "borderpad": 1.2,
            "labelspacing": 1.2,
    ...
    _render.savefig(
        f"images/pval_comp/correlation-c{i}{current_experiment_type}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
...
def get_accuracy(df, identifiers, colx, coly, alpha):
    ...
    df = df.group_by(identifiers).agg(
        (pl.col("tp") + pl.col("tn")).mean().alias("accuracy")
    )
    return df
...
df_fed = get_accuracy(
    df, identifiers, "federated_p_values", "baseline_p_values", alpha
).rename({"accuracy": "Federated"})
df_fisher = get_accuracy(
    df, identifiers, "fisher_p_values", "baseline_p_values", alpha
).rename({"accuracy": "Meta-Analysis"})
...
for i in [1, 3, 5, 7]:
    ...
    _render.savefig(
        f"images/pval_comp/accuracy-c{i}{current_experiment_type}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
