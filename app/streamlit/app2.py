...
if "server_url" not in st.session_state:
    if "LITESTAR_CONTAINER_NAME" in os.environ and "LITESTAR_PORT" in os.environ:
        st.session_state["server_url"] = (
            f"http://{os.environ['LITESTAR_CONTAINER_NAME']}:{os.environ['LITESTAR_PORT']}"
        )
    else:
...
def check_server_connection():
    ...
    if (
        ...
        and (curr_time - st.session_state["last_health_check"]).total_seconds() < 60
    ):
        return True
    try:
...
def step_check_in_to_server():
    ...
    algo_type = col2.selectbox(
        "Algorithm", [e.name for e in env.Algorithm], label_visibility="collapsed"
    )
    ...
    if st.session_state["server_provided_user_id"] is None:
        ...
        request_params = {"username": username, "algorithm": algo_type}
    else:
...
def step_process_data():
        ...
        with st.spinner("Data is being processed..."):
            ...
            with (ro.default_converter + pandas2ri.converter).context():
                ...
                result = run_ci_test_f(
                    df_r, max_conditioning_set, ci_result_dir + "/", fileid
                )
                ...
                if any(
                    [a != b for a, b in zip(labels, st.session_state["data_labels"])]
                ):
                    st.error("""A major issue with the data labels occured.
                                Please reload the page.
                                If this error persists, contact an administrator.
    ...
    if col3.button(
        ...
        disabled=st.session_state["data_pvals"] is None,
        use_container_width=True,
    ):
    ...
    def change_cond_set_value():
        st.session_state["_local_max_conditioning_set"] = st.session_state[
            "local_max_conditioning_set"
        ]
    ...
    if (
        st.session_state["data_pvals"] is not None
...
def step_setup_rpc():
    ...
    hostname = col1.text_input(
        "Hostname:",
        placeholder=st.session_state["rpc_hostname"],
        label_visibility="collapsed",
        key="rpc_hostname",
        on_change=change_hostname,
    )
    ...
    port = col2.number_input(
        "Port:",
        value=st.session_state["rpc_port"],
        step=1,
        label_visibility="collapsed",
        key="rpc_port",
        on_change=change_port,
    )

    if col3.button(
        ":arrow_heading_up:", help="Send RPC details", use_container_width=True
    ):
        r = post_to_server(
...
def main():
    ...
    if st.session_state["selected_algorithm"] == env.Algorithm.META_ANALYSIS:
        ...
        if (
            st.session_state["fedci_client"] is None
            and st.session_state["current_room"] is not None
        ):
            ...
            client_thread = threading.Thread(
                target=client.start, args=(st.session_state["rpc_port"],), daemon=True
            )
            client_thread.start()
            st.session_state["fedci_client"] = client
            st.session_state["fedci_client_thread"] = client_thread
        elif (
            st.session_state["fedci_client"] is not None
            and st.session_state["current_room"] is None
        ):
        ...
        raise Exception(
            f"Unknown algorithm selected: {st.session_state['selected_algorithm']}"
        )
    ...
    if step == 0:
        ...
        if st.session_state["server_provided_user_id"] is None:
            st.info(
                "Please ensure you have checked in with the server before continuing"
            )
            return
        step_upload_data()
    elif step == 2:
