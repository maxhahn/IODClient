...
dir = (
    "experiments/simulation/single_data"  # USE SINGLE DATA TO PLOT DIFF TO REAL P VALUE
)
...
print(
    df.group_by("faithfulness")
    .agg(cs.starts_with("correct_").mean())
    .with_columns(diff=pl.col("correct_fedci") - pl.col("correct_fisher"))
    .sort("faithfulness")
)
print(
...
print(
    df.select(
        pl.col("pvalue_diff_fisher_pooled", "pvalue_diff_fedci_pooled")
        .abs()
        .min()
        .name.suffix("_MIN"),
        pl.col("pvalue_diff_fisher_pooled", "pvalue_diff_fedci_pooled")
...
_df = _df.with_columns(
    max_pvalue_diff=pl.max_horizontal(
        pl.col("pvalue_diff_fisher_pooled").abs(),
        pl.col("pvalue_diff_fedci_pooled").abs(),
    )
)
_df = _df.with_columns(
    max_pvalue_diff_per_test=pl.max("max_pvalue_diff").over(
        "num_samples", "num_splits", "ord", "X", "Y", "S"
    )
)
_df = _df.with_columns(
    adjusted_pvalue_diff_fisher=pl.col("pvalue_diff_fisher_pooled")
    / pl.col("max_pvalue_diff_per_test"),
    adjusted_pvalue_diff_fedci=pl.col("pvalue_diff_fedci_pooled")
    / pl.col("max_pvalue_diff_per_test"),
)
...
for nsamples in df["num_samples"].unique().to_list():
    ...
    _render.savefig(
        f"images/pval_diff/pval_adjusted_diff_to_pooled_box_by_splits-{nsamples}samples-{faithfulness_filter}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
    ...
    _render = hv.render(plot, backend="matplotlib")
    _render.savefig(
        f"images/pval_diff/pval_adjusted_diff_to_pooled_box_by_splits-fliers-{nsamples}samples-{faithfulness_filter}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
...
for nparts in df["num_splits"].unique().to_list():
    ...
    _render = hv.render(plot, backend="matplotlib")
    _render.savefig(
        f"images/pval_diff/pval_adjusted_diff_to_pooled_box_by_samples-{nparts}parts-{faithfulness_filter}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
    ...
    _render = hv.render(plot, backend="matplotlib")
    _render.savefig(
        f"images/pval_diff/pval_adjusted_diff_to_pooled_box_by_samples-fliers-{nparts}parts-{faithfulness_filter}.svg",
        format="svg",
        bbox_inches="tight",
        dpi=300,
    )
